...
- **Amendment (TEACH-36, 2026-09-12) — parent domain.** Point (3) of the first amendment is closed: `app.bresinski.org` → Vercel `teaching-journey-web`, `api.bresinski.org` → Railway `api` (custom domain, CNAME on Vercel DNS), `COOKIE_DOMAIN=.bresinski.org`, `COOKIE_SAMESITE=lax`; `WEB_ORIGIN` and `VITE_API_URL` name the new hosts and `BETTER_AUTH_URL` stays the `${{RAILWAY_PUBLIC_DOMAIN}}` reference, which Railway resolves to the custom domain (`infra/env.contract.ts`). `bresinski.org` is the founder's domain, used until a product domain is bought; the old `*.vercel.app` and `*.up.railway.app` hosts still resolve but are not in `WEB_ORIGIN`.
