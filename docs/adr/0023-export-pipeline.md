...
- Status: Accepted
- Date: 2026-09-06
- Related PRD decisions: TD project items 3 (print and capture routes) and 4 (export leftovers), F12 (archived, built as TeachDeck export)
- Related ADRs: 0004, 0005, 0021, 0022
...
2. **Print routes are SPA routes.** `/w/$worksheetId/print` (existing stub) and a new
   `/l/$lessonId/print` (search: `answers`, `notes`, `auto`, `range`, validated with Zod in the route
   file) render `@tj/editor`'s static slide/sheet renderers in print CSS with no app chrome, under
...
7. **Filenames and options.** TeachDeck's names are kept: `${slug}.teachdeck.json`,
   `${slug}.worksheet.json`, `${slug}.pptx`, `${slug}.docx`, `${slug}-${index}.png`. Every
   format's `includeAnswers` defaults to the same value (TD item 4); the PDF range control stays in
   the PNG panel as `docs/DEFERRED.md` decides.

## Consequences

- Two extra routes in `apps/web` and none anywhere else; `router.test.ts` and the a11y route loop
  gain them. Print pages carry no sidebar, no app bar and no toaster.
...
- Fonts in exported PPTX/DOCX are family names (`FONT_FAMILIES` in `lib/export/pptx.ts`), so the
  eleven theme fonts must be installed on the teacher's machine for full fidelity — unchanged from
  TeachDeck.
