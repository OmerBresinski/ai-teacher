...
- **Commits:** trunk-based development on `master` (amended 2026-09-03 — originally written as `main`; the GitHub repo was created with `master` and is not renamed), squash merges; Conventional Commits enforced by `commitlint` in a `lefthook` pre-commit/commit-msg hook alongside Biome. No changesets.
