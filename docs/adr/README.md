...
- 2026-09-06 — ADR 0018 §1: `@mastra/core` adopted in-process for the lesson pipeline, no Mastra storage, server or model router; price table, `costUsd()`, `createBudget()` and richer log fields in `@tj/ai` (ADR 0025 §15, §16, §21). See `0018-ai-provider.md`.
- 2026-09-06 — ADR 0021 §1: the theme catalogue and layout recipes move to `@tj/slides`; optional `facts`, `generation`, `artefacts`, `sources`, `generatedFrom`, `authoredBy`, `Worksheet.lessonId` (ADR 0025). See the third amendment in `0021-tie-in-document-contract.md`.
...
- 2026-09-06 — ADR 0024 §14, §18: one `lesson.plan` job with checkpoints; the worker writes through `putDocumentAsJob` keyed on the lock; stale locks released on read (ADR 0025 §5–§7, §24). See the amendment in `0024-document-persistence-and-lesson-brief.md`.
