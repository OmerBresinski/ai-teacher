/**
 * Text fitting engine — the pure half. Moved from `@tj/editor/layout` (TEACH-28) so
 * `materialiseSlide` fits a slide with the headless ruler (`./text-measure.ts`) before anything
 * renders it; the editor re-exports it from its old path and still measures with the DOM.
 *
 ...
 *
 * A box standing on the foot of the safe area is the exception to "the author's
 * gap": it is pinned to the foot, so the block above may grow into the space
 * between them and only pushes it once they come within one spacing stop.
 *
...
function layoutPass(slide: Slide, theme: Theme, measure: Measurer, sizes: Map<Id, number>): Slot[] {
  ...
  for (const slot of order) {
    ...
    for (const other of placed) {
      ...
      // A box the author stood on the foot of the safe area (a footnote, a "write your
      // answer" line) is pinned there, not spaced from the block above: its gap is slack
      // the block may grow into, down to one spacing stop. Keeping the whole authored gap
      // pushed an exit ticket's footnote off the slide under a body that still had room.
      if (Math.abs(slot.y0 + slot.h0 - SAFE_BOTTOM) <= 1) gap = Math.min(gap, SPACE[2]);
...
/**
 * Where the box ends once its cushion is counted — what the fit test uses. A box still standing
 * where the author pinned it on the foot of the safe area, no taller than drawn, is exact: the
 * cushion is for text that grew, and counting it there made every recipe footnote an overflow
...
export type ReflowOptions = {
  ...
  fitBottom?: number;
  /**
   * Elements the step-down leaves at their size. `fitSlide` passes a question slide's stem: it
   * sits on the 38pt question floor (SPEC §7), not the 26pt a plain heading may fall to, and
   * shrinking it never buys the cards below it any room, since nothing is pulled up.
   */
  keep?: readonly Id[];
};
