import type { Id, LessonFacts, RecipeId, WorksheetBlock } from "@tj/domain/documents";
import {
  BLOCK_GUIDES,
  FILL_GAP_BANK_INSTRUCTION,
  WORD_SEARCH_MAX_SIZE,
  WORKSHEET_RECIPE_IDS,
 ...
 * makes the frame from `LessonFacts` at once; the model's fill (the `lesson.worksheet` job, ADR
 * 0030 item 3) arrives later and replaces the one placeholder block each recipe leaves for it, so
...
/** The prefix on the paragraph a recipe leaves for the model; `isPlaceholder` finds it again. */
export const PLACEHOLDER_PREFIX = "Generation writes this part: ";

/** A paragraph that says what the model will write in its place. */
const placeholder = (text: string, refs?: string[]): WorksheetBlock =>
  paragraph(`${PLACEHOLDER_PREFIX}${text}`, refs);

/** True for the paragraph a recipe left for the model to replace. */
export function isPlaceholder(block: WorksheetBlock): boolean {
  if (block.type !== "paragraph") return false;
  const first = block.doc.content?.[0]?.content?.[0];
  return typeof first?.text === "string" && first.text.startsWith(PLACEHOLDER_PREFIX);
}
...
/** In catalogue order, the order of `WORKSHEET_RECIPE_IDS` (`recipes.test.ts` holds them equal). */
export const WORKSHEET_RECIPES: readonly WorksheetRecipe[] = [
  exitTicket,
  knowledgeCheck,
  misconceptionCheck,
  cloze,
...
export function isRecipeId(id: string): id is RecipeId {
  return (WORKSHEET_RECIPE_IDS as readonly string[]).includes(id);
}
...
/**
 * The recipe `POST /lessons/:id/worksheet` builds (ADR 0030 item 8): the one named, or the
 * suggestion for these facts when the request says `"auto"`.
 */
export function resolveRecipe(id: RecipeId | "auto", facts?: LessonFacts): WorksheetRecipe {
  const resolved = id === "auto" ? suggestRecipe(facts) : id;
  const recipe = recipeById(resolved);
  if (!recipe) throw new Error(`Unknown worksheet recipe ${resolved}`);
  return recipe;
}

/** The practice time a recipe is built for when the request says `"auto"`: its range's midpoint. */
export function defaultPracticeMinutes(recipe: WorksheetRecipe): number {
  const [low, high] = recipe.minutes;
