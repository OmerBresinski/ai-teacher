...
describe("suggestRecipe / resolveRecipe / defaultPracticeMinutes", () => {
  ...
  test("the default practice time is the midpoint of the recipe's range", () => {
    expect(defaultPracticeMinutes(resolveRecipe("knowledge-check"))).toBe(13);
    expect(defaultPracticeMinutes(resolveRecipe("exit-ticket"))).toBe(5);
    expect(defaultPracticeMinutes(resolveRecipe("exam-style"))).toBe(38);
    for (const recipe of WORKSHEET_RECIPES) {
      const minutes = defaultPracticeMinutes(recipe);
      expect(minutes).toBeGreaterThanOrEqual(recipe.minutes[0]);
      expect(minutes).toBeLessThanOrEqual(recipe.minutes[1]);
    }
  });
});
...
describe("worksheet recipes", () => {
  ...
  test("each recipe leaves at most one placeholder for the model, where the frame says it is", () => {
    for (const recipe of WORKSHEET_RECIPES) {
      for (const withFacts of [true, false]) {
        const built = recipe.build(withFacts ? facts : undefined);
        const placeholders = built.filter(isPlaceholder);
        if (recipe.id === "word-search" || recipe.id === "exam-style") {
          // The seeded grid is exact; the exam paper draws its six from the lesson's questions.
          expect(placeholders).toEqual([]);
        } else if (recipe.id === "worked-example") {
          // "Now try" is left for the model only when the lesson has fewer than three questions.
          expect(placeholders).toEqual([]);
          const short = recipe.build({ ...facts, questions: facts.questions.slice(0, 2) });
          expect(short.filter(isPlaceholder).length).toBe(1);
          expect(isPlaceholder(short.at(-1) as (typeof short)[number])).toBe(true);
        } else if (recipe.id === "reading") {
          expect(placeholders.length).toBe(1);
          expect(built[2]).toBe(placeholders[0] as (typeof built)[number]);
        } else {
          expect(placeholders.length).toBe(1);
          expect(built.at(-1)).toBe(placeholders[0] as (typeof built)[number]);
        }
      }
    }
  });
