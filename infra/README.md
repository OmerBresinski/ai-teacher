...
| `build.watchPatterns`                   | image inputs (`Dockerfile`, `.dockerignore`, `infra/docker/**`, `.railway/**`, manifests) + `packages/<p>/**` for every package in the api ∪ worker transitive `@tj/*` closure, computed from the manifests by `.railway/watch.ts` (TEACH-276; today ai, config, db, domain, extract, generation, images, jobs, slides, storage) + `apps/api/**` | same + `apps/worker/**` |
