...
await runMain(async () => {
  ...
  // -- env files --------------------------------------------------------------------------------
  // Discovery: every `**/.env.example` (excluding node_modules, .git, dist, ...) must have a
  // sibling `.env` containing every active `KEY=` of the example. Replace with the env contract
  // when it lands (TEACH-26).
  const examples = await findEnvExamples(ROOT);
  if (examples.length === 0) {
    await check("Env files", async () => warn("no .env.example files found"));
  }
  for (const example of examples) {
    const envPath = envPathFor(example);
    await check(`Env ${rel(envPath)}`, async () => {
      const diff = await diffEnv(example, envPath);
      if (!diff.envExists) {
        return fail("file missing", `bun run setup   (copies ${rel(example)} -> ${rel(envPath)})`);
      }
      if (diff.missing.length > 0) {
        return fail(
          `missing ${diff.missing.length} key(s): ${diff.missing.join(", ")}`,
          `add them to ${rel(envPath)} -- see ${rel(example)} for values and comments`,
        );
      }
      const extra = diff.extra.length > 0 ? `; extra keys: ${diff.extra.join(", ")}` : "";
      return pass(`all keys of ${rel(example)} present${extra}`);
    });
  }
