...
describe("targets and reports", () => {
  ...
  test("compareNames + fixCommands: missing and extra names, placeholders never values", () => {
    ...
    expect(commands).toEqual([
      ...
      "railway variable set AI_GATEWAY_API_KEY --stdin --service worker --skip-deploys  < /path/to/secret",
      "railway variable set OPENROUTER_API_KEY --stdin --service worker --skip-deploys  < /path/to/secret",
      "railway variable set --service worker --skip-deploys 'AWS_REGION=us-east-1'",
      "railway variable set --service worker --skip-deploys 'AI_MODEL_FRONTIER=us.openai.gpt-5.6-sol'",
      "railway variable set --service worker --skip-deploys 'AI_MODEL_STANDARD=us.openai.gpt-5.6-terra'",
      "railway variable set --service worker --skip-deploys 'AI_MODEL_SMALL=us.openai.gpt-5.6-luna'",
