...
`COOKIE_DOMAIN`, `COOKIE_SAMESITE`, `MAIL_PROVIDER`, `RESEND_API_KEY`, `MAIL_FROM`, `GOOGLE_CLIENT_ID` /
...
`MAIL_PROVIDER=resend` (production, TEACH-35) sends through Resend from `MAIL_FROM` and refuses to
boot without `RESEND_API_KEY` and `MAIL_FROM`; `console` (local, PR environments) prints the link
in the log. Sender: `apps/api/src/mail/resend.ts`.
