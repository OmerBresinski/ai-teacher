...
`docs/env.md`). Production `VITE_API_URL` is `https://api.bresinski.org` (TEACH-36; the app is
`https://app.bresinski.org`); previews use
`RAILWAY_PR_API_URL_TEMPLATE=https://api-ai-teacher-pr-{pr}.up.railway.app`. The API side
(`COOKIE_DOMAIN`/`COOKIE_SAMESITE`, `WEB_ORIGIN_PATTERNS`) is described in `apps/api/README.md`
"Cookie strategy".
