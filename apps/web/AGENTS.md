...
- Tests: `bun test` + React Testing Library + happy-dom; Playwright + axe in `e2e/` (ADR 0014). Biome
  ...
  narrow viewport), `series` (detail page incl. real-pointer drag), `a11y` (every route × the three
  themes via `page.addInitScript` setting `tj-theme`, plus open dialogs/menus), `kit` (opt-in,
