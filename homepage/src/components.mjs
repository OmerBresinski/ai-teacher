...
export const button = (label, route = "/examples/", options = {}) =>
  `<a class="button ${options.secondary ? "button-secondary" : ""}" href="${href(route)}">${label}<span aria-hidden="true">${options.secondary ? "→" : "↗"}</span></a>`;
...
export function shell(page) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${escapeHtml(page.title.replace(/^Gather \| /, "").replace(/ \| Gather$/, ""))} | Gather</title><meta name="description" content="${escapeHtml(page.description)}"><link rel="icon" href="${href("/assets/favicon.svg")}" type="image/svg+xml"><link rel="stylesheet" href="${href("/assets/system.css")}">${["home", "features", "examples", "information"].map((n) => `<link rel="stylesheet" href="${href(`/assets/${n}.css`)}">`).join("")}<link rel="stylesheet" href="${href("/motion/cast.css")}"></head><body data-living-cast><a class="skip" href="#main">Skip to content</a><header class="site-header container"><a class="brand" href="${href("/")}" aria-label="Gather home">gather<span aria-hidden="true">✳</span></a><button class="menu-toggle" aria-expanded="false" aria-controls="main-nav">Menu <span aria-hidden="true">+</span></button><nav id="main-nav" aria-label="Main navigation">${nav.map(([label, route]) => `<a href="${href(route)}" ${page.route === route ? 'aria-current="page"' : ""}>${label}</a>`).join("")}<a class="nav-cta" href="${href("/examples/shadows/")}">Explore a lesson <span aria-hidden="true">↗</span></a></nav></header><main id="main">${page.body}</main><footer class="site-footer"><div class="container"><div class="footer-top"><div><a class="brand" href="${href("/")}">gather<span aria-hidden="true">✳</span></a><p>Lesson preparation for teachers.</p><p class="preview-note">A working preview. Explore prepared lessons;<br>live creation and accounts aren’t connected yet.</p></div><div class="footer-group"><h2>The materials</h2>${[
