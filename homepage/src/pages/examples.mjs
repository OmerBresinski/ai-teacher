...
function lesson(slug, d) {
  return `<section class="ex-lesson-head"><a class="ex-back" href="${href("/examples/")}">← All sample lessons</a><p class="eyebrow">${d.year} science · 40 minutes</p><h1>${d.title}</h1><p>${d.intro}</p><p class="ex-disclosure">Prepared sample lesson · Explore the materials below.</p></section><div class="ex-workspace" data-example="${slug}"><div class="ex-tabs" role="tablist" aria-label="Lesson materials">${["plan", "slides", "worksheet", "answers", "review"].map((k, i) => `<button id="tab-${k}" role="tab" aria-selected="${i === 0}" aria-controls="${k}" tabindex="${i === 0 ? 0 : -1}" data-tab="${k}">${k === "review" ? "Lesson review" : k[0].toUpperCase() + k.slice(1)}</button>`).join("")}</div>${section("plan", "The lesson plan", `<div class="ex-plan-top"><div><p class="eyebrow">Learning goal</p><p class="ex-goal">${d.goal}</p></div>${character("support", { className: "ex-helper" })}</div><ol class="ex-timings">${d.plan.map(([time, title, body]) => `<li><span>${time}</span><div><h3>${title}</h3><p>${body}</p></div></li>`).join("")}</ol><aside class="ex-note"><h3>Teacher notes</h3><p>${d.notes}</p></aside><details class="ex-brief"><summary>The brief behind this lesson</summary><p>“${d.brief}”</p></details>`)}${section("slides", "The teaching slides", `<div class="ex-slide-viewer">${d.slides.map(([title, body, visual], i) => `<article class="ex-slide" data-slide="${i}"${i ? " hidden" : ""}><p class="ex-slide-label">${d.year} science</p><h3>${title}</h3>${visual}<p>${body}</p></article>`).join("")}<div class="ex-slide-controls"><button type="button" data-slide-prev aria-label="Previous slide">←</button><span data-slide-count aria-live="polite">1 of ${d.slides.length}</span><button type="button" data-slide-next aria-label="Next slide">→</button></div></div><p class="ex-small">Prepared slide preview. Use the arrows to move through the explanation.</p>`)}${section("worksheet", "A little independent thinking", `<div class="ex-print-head"><div><p>${d.title}</p><p class="ex-name">Name: ____________________ &nbsp; Date: __________</p></div><button class="ex-print" data-print="worksheet" type="button">Print worksheet ↗</button></div><p>Use the lesson and your observations to answer these questions.</p><ol class="ex-questions">${d.questions.map((q) => `<li><p>${q}</p><div class="ex-answer-space" aria-hidden="true"></div></li>`).join("")}</ol><aside class="ex-word-bank"><strong>Words to help</strong><p>${d.bank}</p></aside>`)}${section("answers", "The teacher’s copy", `<div class="ex-print-head"><p>Suggested answers · ${d.title}</p><button class="ex-print" data-print="answers" type="button">Print answers ↗</button></div><ol class="ex-answer-key">${d.answers.map((a, i) => `<li><h3>${d.questions[i]}</h3><p>${a}</p></li>`).join("")}</ol><p class="ex-small">Accept equivalent explanations that demonstrate the learning goal. Review the material for your class before teaching.</p>`)}${section("review", "A closer look before teaching", `<p class="ex-review-disclosure">An illustrative review, prepared with this sample. These findings are authored examples, not the output of a live AI check.</p><div class="ex-correction"><p class="eyebrow">Example correction · ${slug === "shadows" ? "Slide 3" : "Slide 2"}</p><h3>${slug === "shadows" ? "Make the investigation precise" : "Correct the particle explanation"}</h3><div class="ex-before-after"><div><span>Before</span><p>${d.reviewBefore}</p></div><div><span>After</span><p>${d.reviewAfter}</p></div></div><p>${d.reviewWhy}</p><a href="#slides" data-open-slides="${slug === "shadows" ? 2 : 1}">See the corrected slide ↗</a></div><aside class="ex-note"><h3>For your judgement</h3><p>${d.reviewAttention}</p></aside><p class="ex-small">A review can miss things. You decide what is suitable for your class.</p>`)}</div><div class="ex-after-sample"><p>From first explanation to the last answer.</p>${button("Explore the other lesson", `/examples/${slug === "shadows" ? "states-of-matter" : "shadows"}/`, { secondary: true })}</div>`;
}
const pages = [
  {
    route: "/examples/",
    ...
    body:
      ...
      `<section class="ex-grid" aria-label="Prepared sample lessons">${Object.entries(data)
        .map(
          ([slug, d]) =>
            `<article class="ex-card"><a class="ex-card-visual" href="${href(`/examples/${slug}/`)}" aria-label="Explore ${d.title}">${d.diagram}</a><div class="ex-card-copy"><p class="eyebrow">${d.year} science · 40 minutes</p><h2><a href="${href(`/examples/${slug}/`)}">${d.title}</a></h2><p>${slug === "shadows" ? "Explore how a shadow changes when the light source moves. Includes a practical activity and a word bank." : "Use the particle model to explain shape, flow and compression. Includes a comparison task and matching answers."}</p>${button("Open the lesson", `/examples/${slug}/`, { secondary: true })}</div></article>`,
        )
        .join(
          "",
